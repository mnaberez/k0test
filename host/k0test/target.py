...
class BaseTarget(object):
    '''Methods a target must implement'''
...
class EmulatorTarget(BaseTarget):
    '''Target using the k0emu software emulator'''
    def __init__(self, processor):
        self.proc = processor
        self.proc.write_sp(0xfe1d)  # for consistency with hardware target firmware
...
class HardwareTarget(BaseTarget):
    '''Target using a real uPD78F0831Y running the debugger firmware'''
...
def make_hardware_target(ser=None):
    if ser is None:
        ser = make_serial()
    return HardwareTarget(ser)

def make_emulator_target(proc=None):
    if proc is None:
        proc = Processor()
    return EmulatorTarget(proc)

def make_target_from_argv(argv=None):
    if argv is None:
        argv = sys.argv
    if 'hardware' in argv:
        return make_hardware_target()
    elif 'emulator' in argv:
        return make_emulator_target()
    else:
        sys.stderr.write("Usage: k0test-run <hardware|emulator>\n")
        sys.exit(1)
